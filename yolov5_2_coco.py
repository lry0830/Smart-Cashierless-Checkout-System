...
class YOLOV5ToCOCO(object):
    def __init__(self, dir_path):
        ...
        self.licenses = [{
            ...
            'url': 'https://github.com/lry0830/Smart-Cashierless-Checkout-System/LICENSE',
        }]
    ...
    def gen_dataset(self, img_paths, target_img_path, target_json):
        ...
        for img_id, img_path in enumerate(img_paths, 1):
            ...
            label_path = str(img_path.parent.parent
                             / 'txt' / f'{img_path.stem}.txt')
            mask_path = str(img_path.parent.parent
                             / 'masks' / f'{img_path.stem}.jpg')
            print(mask_path)

            imgsrc = cv2.imread(str(img_path))
            masksrc = cv2.imread(str(mask_path))
            height, width = imgsrc.shape[:2]

            dest_file_name = f'{img_id:012d}.jpg'
            dest_mask_name = f'{img_id:012d}.png'
            save_img_path = target_img_path / dest_file_name
            save_mask_path = target_img_path / dest_mask_name
            ...
            if img_path.suffix.lower() == ".jpg":
                ...
                cv2.imwrite(str(save_mask_path), masksrc)
